...
export default class Bezier {
  ...
  static getNewSegment(
    ...
    const t0 = this.getDistancePerc(startPerc, bezierData)
    // TODO:
    // eslint-disable-next-line no-param-reassign
    endPerc = endPerc > 1 ? 1 : endPerc
    const t1 = this.getDistancePerc(endPerc, bezierData)
    const u0 = 1 - t0
    const u1 = 1 - t1
    ...
    for (let i = 0; i < pt1.length; i++) {
      ...
      this.bezierSegmentPoints[i * 4 + 1] =
        Math.round(
          (u0u0u1 * pt1[i] +
            t0u0u1_3 * pt3[i] +
            t0t0u1_3 * pt4[i] +
      ...
      this.bezierSegmentPoints[i * 4 + 2] =
        Math.round(
          (u0u1u1 * pt1[i] +
            t0u1u1_3 * pt3[i] +
            t0t1u1_3 * pt4[i] +
      ...
      this.bezierSegmentPoints[i * 4 + 3] =
        Math.round(
          (u1u1u1 * pt1[i] +
            t1u1u1_3 * pt3[i] +
            t1t1u1_3 * pt4[i] +
    ...
    return this.bezierSegmentPoints
  }

  static getPointInSegment(
    pt1: Vector2,
    pt2: Vector2,
    pt3: Vector2,
    pt4: Vector2,
    ...
    const t1 = this.getDistancePerc(percent, bezierData)
    const u1 = 1 - t1
    const ptX =
      Math.round(
        (u1 * u1 * u1 * pt1[0] +
  ...
  static getSegmentsLength(shapeData: ShapePath) {
    ...
    let totalLength = 0
    for (i = 0; i < len - 1; i++) {
      lengths[i] = this.getBezierLength(
        pathV[i] as Vector2,
  ...
  private static getDistancePerc(perc: number, bezierData: any) {
