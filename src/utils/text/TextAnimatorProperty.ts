  ...
  getMeasures(documentData: DocumentData, lettersChangedFlag?: boolean) {
    ...
    if (this._hasMaskedPath) {
      ...
      if (!this._pathData.n || this._pathData._mdf) {
        ...
        totalLength = 0
        for (i = 0; i < len; i += 1) {
          bezierData = TextAnimatorProperty.bez.buildBezierData(
            paths.v[i],
            paths.v[i + 1],
...
class TextAnimatorDataProperty {
  ...
  s: any
  constructor(elem: any, animatorProps: TextData, container: any) {
    ...
    this.s = new TextSelectorPropFactory(
      elem,
      animatorProps.s
      // container
    )
...
class TextSelectorPropFactory extends DynamicPropertyContainer {
  ...
  sm: any
  xe: any
  constructor(elem: any, data: TextRangeValue) {
    super()
