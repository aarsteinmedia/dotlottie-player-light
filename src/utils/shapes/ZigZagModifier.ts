...
function zigZagCorner(
  ...
  const point = path.v[cur % Number(path._length)]
  const prevPoint = path.v[cur === 0 ? Number(path._length) - 1 : cur - 1]
  const nextPoint = path.v[(cur + 1) % Number(path._length)]
  const prevDist =
    pointType === 2
      ? Math.sqrt(
...
ZigZagModifier.prototype.processShapes = function (_isFirstFrame: boolean) {
  let shapePaths
  let i
